...
function Backup({ year }) {
  ...
  async function ftpUpload(){
    try{
      const r=await api('/backup/ftp-upload',{
        method:'POST',
        body:JSON.stringify({year})
      });

      alert('Backup uploaded to FTP server: ' + (r.fileName || 'Success'));
  ...
  async function restoreFile(e){
    ...
    if(!confirm('Restore this backup into database?'))
      return;

    const text=await f.text();
    const payload=JSON.parse(text);

    const r=await api('/backup/upload',{
      method:'POST',
  ...
  return (
    <section className="card">
      ...
      <p>
        Backup encryption:
        <b> {status.encryption || 'AES-256-GCM'}</b>
      ...
      <button onClick={()=>download(year)}>
        <Database size={16}/>
        Download Encrypted Backup ({year})
      </button>

      <button onClick={()=>download('all')}>
        Download Encrypted Backup (All Years)
      </button>
      ...
      <button onClick={ftpUpload}>
        Upload to FTP Server
      </button>

      <p className="muted">
        FTP Host: 185.27.134.11
        <br/>
        Backup Folder: School_management_data
      </p>
      ...
      <p>
        Copy all active students from a previous year
        to the current year ({year}).
      </p>
      ...
      <div className="toolbar">
        ...
        <button onClick={async()=>{
          ...
          const from =
            document.getElementById('migrateFrom').value;
          ...
          if(confirm(`Migrate students from ${from} to ${year}?`)){
            ...
            try{
              ...
              alert(
                `Successfully migrated ${r.migrated} students.`
              );
